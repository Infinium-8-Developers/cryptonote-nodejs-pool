	...
	if (rpcMode === 'getinfo') {
		apiInterfaces.rpcDaemon('getinfo', {}, function (error, reply) {
			if (error || !reply) {
				...
				if (config.cnAlgorithm==="ethash" && config.daemonType=== "default") {
					callback(null, {	
						difficulty: reply.pow_difficulty,
						height: reply.height
					});
				} else {
					callback(null, {
...
function getLastBlockData (callback) {
	apiInterfaces.rpcDaemon('getlastblockheader', {}, function (error, reply) {
		...
		callback(null, {
			...
			height: blockHeader.height,
			timestamp: blockHeader.timestamp,
			reward: config.reward - ((config.blockUnlocker.fixedFee * config.coinUnits) || 0),
			hash: blockHeader.hash
		});
		} else {
			callback(null, {
